                            ...
                            @foreach ($payrolls as $payroll)
                                <tr class="border-b dark:border-gray-700">
                                    <th scope="row"
                                        ...
                                        {{ $payroll->first_name . ' ' . $payroll->last_name }}</th>
                                    <td class="px-4 py-3">{{ $payroll->department->name }}</td>
                                    <td class="px-4 py-3">{{ $payroll->email }}</td>
                                    <td class="px-4 py-3 text-gray-500">
                                        @foreach ($payroll->getRoleNames() as $role)
                                            @switch($role)
                                                @case('superadmin')
                                    ...
                                    <td class="px-4 py-3 text-green-500">
                                        @foreach ($payroll->Permissions() as $permission)
                                            <span>{{ $permission }}</span><br>
                                        @endforeach
                                    </td>
                                    <td class="px-4 py-3">
                                        {{ !$payroll->created_at ? '' : $payroll->created_at->diffForHumans() }}</td>
                                    <td class="px-4 py-3">
                                        {{ !$payroll->updated_at ? '' : $payroll->updated_at->diffForHumans() }}</td>
